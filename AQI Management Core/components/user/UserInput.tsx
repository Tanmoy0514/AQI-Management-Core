    ...
    <div className="max-w-4xl mx-auto w-full space-y-6 animate-in fade-in slide-in-from-left-4">
      ...
      <div className={`p-6 rounded-2xl shadow-lg border ${cardBg}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={userName} onKeyDown={handleNameKeyDown} onChange={(e) => setUserName(e.target.value)} placeholder="Your Name" className={`w-full p-3 rounded-xl border outline-none text-sm ${inputBg}`} />
          </div>
          <div>
            <label className="text-xs font-bold uppercase ml-1 block mb-1 opacity-60">Age</label>
            <input type="number" value={userAge} ref={ageRef} onKeyDown={handleAgeKeyDown} onChange={(e) => setUserAge(e.target.value)} placeholder="Years" className={`w-full p-3 rounded-xl border outline-none text-sm ${inputBg}`} />
          </div>
        </div>
      </div>
      {/* BOX 2: Role */}
      <div className={`p-6 rounded-2xl shadow-lg border ${cardBg}`}>
        ...
        <div>
          ...
          <input type="text" value={institutionName} ref={detailsRef} onKeyDown={handleDetailsKeyDown} onChange={(e) => setInstitutionName(e.target.value)} placeholder={`Enter details...`} className={`w-full p-3 rounded-xl border outline-none text-sm ${inputBg}`} />
        </div>
      </div>
      {/* BOX 3: Health */}
      <div className={`p-6 rounded-2xl shadow-lg border ${cardBg}`}>
