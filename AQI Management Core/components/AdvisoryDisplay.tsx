    ...
    return (
      <div className={`h-full min-h-[500px] flex flex-col items-center justify-center rounded-3xl border-2 border-dashed ${darkMode ? 'border-stone-800' : 'border-slate-200'}`}>
        <div className={`w-32 h-32 rounded-full flex items-center justify-center mb-6 ${darkMode ? 'bg-stone-900' : 'bg-slate-100'}`}>
          <Wind className={`w-16 h-16 ${darkMode ? 'text-stone-700' : 'text-slate-300'}`} />
        </div>
        <p className={`${darkMode ? 'text-stone-500' : 'text-slate-400'} font-medium`}>Complete your profile to generate analysis</p>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-6">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className={`text-xs ${darkMode ? 'text-stone-400' : 'text-slate-400'}`}>Live in {selectedCity.name}</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
              <div className={`p-4 rounded-2xl ${darkMode ? 'bg-stone-800' : 'bg-slate-50'}`}>
                <div className="flex items-center gap-2 mb-1 text-blue-500">
                  <Shield className="w-5 h-5" />
                  <span className="font-bold text-sm">Recommended Mask</span>
                </div>
                <p className={`text-lg font-semibold ${darkMode ? 'text-white' : 'text-slate-800'}`}>{advisory.mask}</p>
              ...
              <div className={`p-4 rounded-2xl ${darkMode ? 'bg-stone-800' : 'bg-slate-50'}`}>
                ...
                <p className={`text-lg font-semibold ${darkMode ? 'text-white' : 'text-slate-800'}`}>{advisory.timeLimit}</p>
          ...
          {/* Visual Mask Graphic */}
          <div className={`hidden md:flex flex-col items-center justify-center p-6 rounded-2xl ${darkMode ? 'bg-stone-800' : 'bg-blue-50'} min-w-[140px]`}>
          ...
          {forecast.map((day, idx) => {
            ...
            return (
              <div key={idx} className="flex-1 min-w-[80px] flex flex-col items-center group">
                ...
                {/* Bar */}
                <div className={`w-full h-24 rounded-xl relative overflow-hidden transition-all hover:scale-105 ${darkMode ? 'bg-stone-800' : 'bg-slate-100'}`}>
                    <div 
